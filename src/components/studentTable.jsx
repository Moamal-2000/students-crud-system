...
function StudentTable({ student }) {
  ...
  let lectures = Array(4)
    .fill(0)
    .map((ele) => student.days.split(" ,"))
    .flat();
  ...
  return (
    <form className="table-container" onSubmit={submitFunc}>
                          ...
                          <span key={order}>
                            {
                              <input
                                type="checkbox"
                                onChange={() => valueChanger(order)}
                                checked={orderValue == "on"}
                                name={order}
                              />
                            }
                          </span>
      ...
      <TableDetails tableData={tableData} student={student} />
